#!/bin/bash
################################################################################
# Submit APPNP Baseline Jobs for Hard Negatives v4
#
...
#
# Usage:
#   bash scripts/submit_appnp_hard_negatives.sh [options]
#
# Options:
#   --queue QUEUE        LSF queue               (default: normal)
#   --walltime TIME      Wall time per job       (default: 1:00)
...
RESULTS_DIR="/dccstor/boseukb/Q/NetMed/QuVINE/results/hard_negatives_v4/results"
LOG_DIR="/dccstor/boseukb/Q/NetMed/QuVINE/results/hard_negatives_v4/logs_appnp"
...
# ── Banner ─────────────────────────────────────────────────────────────────────
echo "============================================================"
echo " APPNP Baseline - Hard Negatives v4"
echo "============================================================"
...
for graphml_path in "${GRAPHML_FILES[@]}"; do
    ...
    # Determine network type from network_id
    if [[ "$network_id" == QW* ]] || [[ "$network_id" == NC* ]] || [[ "$network_id" == RN* ]]; then
        # Extract base case name (e.g., QW1_modular_strong from QW1_modular_strong_rep00)
        case_name="${network_id%_rep*}"
        
        # Determine network type
        if [[ "$case_name" == *"modular"* ]]; then
            network_type="modular"
        elif [[ "$case_name" == *"scale_free"* ]] || [[ "$case_name" == *"barabasi"* ]]; then
            network_type="scale_free"
        elif [[ "$case_name" == *"core_periphery"* ]]; then
            network_type="core_periphery"
        elif [[ "$case_name" == *"ws"* ]] || [[ "$case_name" == *"watts_strogatz"* ]]; then
            network_type="watts_strogatz"
        elif [[ "$case_name" == *"erdos_renyi"* ]]; then
            network_type="erdos_renyi"
        elif [[ "$case_name" == *"random_geometric"* ]]; then
            network_type="random_geometric"
        elif [[ "$case_name" == *"sbm"* ]] || [[ "$case_name" == *"stochastic_block"* ]]; then
            network_type="stochastic_block_model"
        elif [[ "$case_name" == *"powerlaw"* ]]; then
            network_type="powerlaw_cluster"
        elif [[ "$case_name" == *"karate"* ]]; then
            network_type="real_karate"
        elif [[ "$case_name" == *"miserables"* ]]; then
            network_type="real_lesmis"
        elif [[ "$case_name" == *"polbooks"* ]]; then
            network_type="real_polbooks"
        else
            network_type="unknown"
        fi
        
        # Determine negative strategy
        if [[ "$case_name" == QW* ]]; then
            negative_strategy="hard_2hop"
        else
...
# ── Aggregation job ────────────────────────────────────────────────────────────
agg_job_name="appnp_hn_aggregate"

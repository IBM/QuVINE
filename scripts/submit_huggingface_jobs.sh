...
#
# Processes graph datasets generated from the Hugging Face notebook.
...
#
# Usage:
#   bash scripts/submit_huggingface_jobs.sh [--dry-run] [--queue QUEUE]
...
DATA_DIR="/dccstor/cgq4hls/Q/hugging-graph-processed-data/huggingface_graph_samples"
HPARAM_BASE="/dccstor/boseukb/Q/NetMed/QuVINE/results/hparam_tuning"
OUTPUT_BASE="/dccstor/cgq4hls/Q/hugging-graph-processed-data/results"
...
# ---------------------------------------------------------------------------
echo "Discovering graph files in ${DATA_DIR}..."
...
echo "Found ${TOTAL_JOBS} graph files"

# ---------------------------------------------------------------------------
# Banner
# ---------------------------------------------------------------------------
echo "======================================================"
echo " Hugging Face Graph Dataset Job Submission"
echo "======================================================"
...
# ── Load graph ────────────────────────────────────────────────────────────
print('Loading graph from ${CSV_PATH}')
...
if n_before > len(df):
    print(f'Dropped {n_before - len(df)} rows with non-numeric node IDs')
...
# Try to use tuned hyperparameters from similar graph types
method_hyperparams = {}
hparam_candidates = [
...
for hparam_path in hparam_candidates:
    try:
        with open(hparam_path) as f:
            hp = json.load(f)
        # Use first available network's params as baseline
...
# Fix view constraints for quantum walks (same as PPI v4)
...
echo "Aggregating Hugging Face graph results ..."

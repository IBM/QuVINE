#!/bin/bash
################################################################################
# BioSNAP Dataset Job Submission
#
# Processes QuVINE-compatible CSV/JSON graph bundles generated from
# notebooks/biosnap_preprocessing.ipynb.
#
...
#
# Usage:
#   bash scripts/submit_biosnap_jobs.sh [--dry-run]
...
OUTPUT_BASE="/dccstor/cgq4hls/Q/biosnap/results"
...
echo "======================================================"
echo " BioSNAP Dataset Job Submission"
echo "======================================================"
...
print(f'Dataset: {graph_metadata.get("dataset_name", "biosnap")}')
print(f'Type: {graph_metadata.get("type", "biosnap_subsample")}')
...
analysis_metadata = {
    'type': graph_metadata.get('type', 'biosnap_subsample'),
    'graph_name': GRAPH_NAME,
    'network_id': graph_metadata.get('network_id', GRAPH_NAME),
    'dataset_family': 'biosnap',
    'dataset_name': graph_metadata.get('dataset_name', 'biosnap'),
    'csv_path': CSV_PATH,
    'json_path': JSON_PATH,
    'seeds': seeds,
    'targets': targets,
...
AGG_NAME="biosnap_${SIZE_TAG}_aggregate"
...
echo "Aggregating BioSNAP results ..."

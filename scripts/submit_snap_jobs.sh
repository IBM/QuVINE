...
DATA_ROOT="/dccstor/cgq4hls/Q/snap"
HPARAM_BASE="/dccstor/boseukb/Q/NetMed/QuVINE/results/hparam_tuning"
OUTPUT_BASE="/dccstor/cgq4hls/Q/snap/results"
...
echo "Discovering SNAP graph files in ${DATA_ROOT}..."
...
if [ $TOTAL_JOBS -eq 0 ]; then
    echo "ERROR: No CSV files found in ${SEARCH_PATH}"
    exit 1
fi
...
echo "======================================================"
echo " SNAP Dataset Job Submission"
echo "======================================================"
...
print(f'Dataset: {graph_metadata.get("dataset_name", "snap")}')
print(f'Type: {graph_metadata.get("type", "snap_subsample")}')
...
analysis_metadata = {
    'type': graph_metadata.get('type', 'snap_subsample'),
    'graph_name': GRAPH_NAME,
    'network_id': graph_metadata.get('network_id', GRAPH_NAME),
    'dataset_family': 'snap',
    'dataset_name': graph_metadata.get('dataset_name', 'snap'),
    'csv_path': CSV_PATH,
    'json_path': JSON_PATH,
    'seeds': seeds,
    'targets': targets,
...
AGG_NAME="snap_${SIZE_TAG}_aggregate"

#!/bin/bash
################################################################################
# TRRUST v2 TF-Target Network Job Submission
#
# Processes TRRUST v2 TF-target interaction graphs generated from the notebook.
...
#
# Usage:
#   bash scripts/submit_trrust_jobs.sh [--dry-run] [--queue QUEUE]
...
DATA_DIR="/dccstor/cgq4hls/Q/trrust_human_graph_samples"
HPARAM_BASE="/dccstor/boseukb/Q/NetMed/QuVINE/results/hparam_tuning"
OUTPUT_BASE="/dccstor/cgq4hls/Q/trrust_results"
...
# ---------------------------------------------------------------------------
echo "Discovering TRRUST graph files in ${DATA_DIR}..."
...
echo "Found ${TOTAL_JOBS} TRRUST graph files"

# ---------------------------------------------------------------------------
# Banner
# ---------------------------------------------------------------------------
echo "======================================================"
echo " TRRUST v2 TF-Target Network Job Submission"
echo "======================================================"
...
try:
    ...
    print(f'Loaded TRRUST metadata from {metadata_path}')
    print(f'Metadata keys: {list(graph_metadata.keys())}')
    print(f'Species: {graph_metadata.get("species", "unknown")}')
except Exception as e:
    print(f'WARNING: Could not load metadata: {e}')
    graph_metadata = {}

# ── Load graph ────────────────────────────────────────────────────────────
print('Loading TRRUST graph from ${CSV_PATH}')
...
# Build graph (TRRUST uses gene symbols, keep as strings)
...
echo "Aggregating TRRUST results ..."
